...
class MetaDataReaderImpl(private val application: Application) : MetaDataReader {
    override fun getMetaDataFromUri(contentUri: Uri): MetaData? {
        ...
        return fileName?.let {fileFullName ->
            MetaData(fileName = Uri.parse(fileFullName).lastPathSegment ?: return null)
        }
    }
